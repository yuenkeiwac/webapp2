...
st.header("Welcome to YK Food Menu Restaurant")
...
if readme:
    ...
    st.write("""
        This is a simple binary search tree demo using [streamlit](https://streamlit.io/) library. It is hosted on [heroku](https://www.heroku.com/). You may get the codes via [github](https://github.com/yuenkeiwac/binarySearchTreeDemo)
        """)
...
if option == 'Add a food':
    ...
    if submit:
        ...
        if foodName != "" and foodPrice !="":
            ...
            if st.session_state.hasFood==1:
                ...
                st.session_state.YKFoodMenu.addNode(foodName,foodPrice)
                st.write("[{} , RM {}] has been added to the food Menu.".format(foodName, foodPrice))
            
            else:
                st.session_state.YKFoodMenu = foodMenu(foodName,foodPrice)
                st.write("[{} , RM {}] has been added to the food Menu.".format(foodName, foodPrice))
    ...
    if st.session_state.hasFood ==1:
        ...
        st.write("The food is in the Food Menu: {}".format(st.session_state.YKFoodMenu.findNode(foodName)))
    ...
    if st.session_state.hasFood ==1:
        ...
        st.write(pd.DataFrame(st.session_state.YKFoodMenu.inOrderTraversal(),columns=["Food Name", "Price"]))
